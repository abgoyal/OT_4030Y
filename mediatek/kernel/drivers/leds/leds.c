...
#else
#define INDICATOR_GPIO_NUM	GPIO194
#endif
/* added by songlinhua 20121212 end */
...
static int backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
 ...
 //ygm added for beetlelite JB backlight control. 20121212.
 #if defined(BEETLELITE_JB)
		...
		case MT65XX_LED_MODE_PWM:
			if(strcmp(cust->name,"lcd-backlight") == 0)
			{
              //ygm added for beetlelite JB backlight control. 20121212.
                ...
                bl_duty = level;
              #else
				mt_set_gpio_mode(GPIO66, GPIO_MODE_01);//added by xiewei
				bl_brightness = level;
				if(level == 0)
				{
					backlight_flag = 1; // added by leibo
					upmu_boost_en(0x0);//added by xiewei
					mt_set_pwm_disable(cust->data);
					mt_pwm_power_off (cust->data);
				}else
				{	
					ret = backlight_set_pwm(cust->data, level, bl_div,&cust->config_data);
					if(ret != 0)
						return 1;
					if (backlight_flag == 1) // added by leibo for avoiding screen blink when light on/off . 
					{
						msleep(30);
						backlight_flag = 0;
					}
			...
			}else
			{
				if(level == 0)
				{
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ZERO);//for power saving
						}
				...
				}else
				{
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ONE);//for power saving
						}
	...
	if(buf != NULL && size != 0)
		{
			...
			if(notify_led == 1)
				{
					mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ONE);
					notify_led_enable = 1;
			...
			if(notify_led == 0)
				{
					mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ZERO);
					notify_led_enable = 0;
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if(strcmp(g_leds_data[i]->cdev.name,"red")==0)
		{
			...
			mt_set_gpio_mode(INDICATOR_GPIO_NUM, GPIO_MODE_00);
			mt_set_gpio_dir(INDICATOR_GPIO_NUM, GPIO_DIR_OUT);
			mt_set_gpio_out(INDICATOR_GPIO_NUM, GPIO_OUT_ZERO);
			mt65xx_led_set_cust(&cust_led_list[0], 0);//add by chenfei,the initial state set to led off
		}
		if (ret)
			goto err;
